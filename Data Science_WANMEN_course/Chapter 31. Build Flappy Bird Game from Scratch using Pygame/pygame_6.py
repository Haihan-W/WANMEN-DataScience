...
#get player(bird)'s height info for later use.
...
playerAccY=2 # player downward acceleration due to gravity
...
while True:
	for event in pygame.event.get():
		if event.type == QUIT:
			exit()
# Remove the following section for now, since this pygame is intended to randomly flap the bird without human inputs.------------------------			
	...
	# 			move_x = 3
	# 		elif event.key == K_UP: #upward arrow
	# 			move_y = -3
